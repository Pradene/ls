...
static void quicksort_swap(void *a, void *b, size_t size) {
    char temp;
    char *p = a;
    char *q = b;
...
static ptrdiff_t quicksort_partition(char *base, ptrdiff_t low, ptrdiff_t high, size_t size, int (*comp)(const void *, const void *)) {
    ptrdiff_t mid = low + (high - low) / 2;
    char *pivot = base + mid * size;
    quicksort_swap(pivot, base + high * size, size);
    ...
    for (ptrdiff_t j = low; j < high; ++j) {
        ...
        if (comp(current, pivot) <= 0) {
            ++i;
            quicksort_swap(current, base + i * size, size);
        }
    }

    quicksort_swap(base + (i + 1) * size, pivot, size);
    return (i + 1);
}

static void quicksort_recurse(char *base, ptrdiff_t low, ptrdiff_t high, size_t size, int (*comp)(const void *, const void *)) {
    if (low < high) {
        ptrdiff_t pivot = quicksort_partition(base, low, high, size, comp);
